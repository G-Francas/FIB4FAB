...
def Check(res,er, material, gType):
    duty=0.5
    arr=np.ones([res,res])
    arrIm=arr
    arr=(1.0006+0j)*arr
...
def CheckErr(res,er, material, gType, err):
    duty=0.5
    err=float(err)
    arr=np.ones([res,res])
    arrIm=arr
...
def CheckCoat(res,er, material, gType,si):
    duty=0.5
    arr=np.ones([res,res])
    arr=(1.000536+0j)*arr
    i=0
...
def CheckDiag(res,er, material, gType):
    arr=np.ones([res,res])
    arrIm=500*arr
    arr=(1.000536+0j)*arr
    i=0
...
def CheckDiagErr(res,er, material, gType,err):
    err=float(err)
    cells=int(err*res/2)
