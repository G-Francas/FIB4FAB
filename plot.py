...
def plotter(firstxs,firstys,zeros,diags,R,loop, gType, tests,count, profile, slope, depthF, nLayers, res, material, wavelength, period, har,val):
    #plt.figure()
    #plt.imshow(R)
    #plt.colorbar(label="intensity")
    #plt.title("Reflected diffraction pattern")
    ...
    if profile:
        img=makeProf(slope,depthF,nLayers,res, material, gType)
        plt.figure()
        plt.imshow(img)
        display.display(plt.gcf())
    return 
